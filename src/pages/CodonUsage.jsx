...
const CodonUsage = () => {
  ...
  const generateCodonTableData = (counts) => {
    ...
    const totalCodonCount = Object.values(counts).reduce((sum, count) => sum + count, 0);

   return Object.entries(codonTable).map(([codon, aminoAcid]) => {
        const count = counts[codon] || 0;
        const frequency = totalCodonCount > 0 ? (count / totalCodonCount) * 100 : 0;
  ...
  return (
    <Container className="container py-4">
      <h2 className="text-center mb-4 title">Codon Usage Calculator</h2>
      ...
      {result && <p className="mt-3 result">Codon Usage: { generateCodonTable(result) }</p>}
    </Container>
  );
};
